...
- `quorumRequiredCount` and `approvalRequiredCount` governance thresholds
...
- explicit `ratifyPriorActions` with no schema default
...
- vote: `FOR`, `AGAINST`, `ABSTAIN`, `NOT_VOTING`, or `RECUSED`; absence requires `NOT_VOTING`
- disposition: `APPROVED_UNANIMOUSLY`, `APPROVED_REQUIRED_VOTE`, or `NOT_APPROVED`

The renderer derives exact human-readable language and vote rollups from these values. Meeting quorum and approval disposition are validated against the reviewed thresholds instead of a hardcoded majority. Delivery recipient and condition must either both be supplied or both omitted. `certificateDate` cannot precede `actionDate`. A `NOT_APPROVED` decision cannot carry ratification or delivery authority. Director assignment indexes must be `0`, `1`, or `2`.
...
- The selected Secretary director receives an additional Secretary signature and execution-date field; `certificateDate` remains the separately printed effective date of the certificate.
...
- The durable authorization signer roster remains authoritative. Envelope status synchronization may add status metadata but rejects changed, missing, reordered, role-changed, or additional recipients.

## UI And Automation

- Use select controls for action method, presence, vote, disposition, thresholds, and director role assignments; use an extensible suggestion menu for entity type and a checkbox for prior-action ratification.
...
- Sending requires a separately generated READY envelope so the generated PDF and field layout can be reviewed before the explicit send action.
