  ...
  return (
    <div className="grid gap-4">
      {signerSlots.map((slot) => {
        ...
        const defaultPresence = director?.presence ?? (templateVersion === 1 ? 'Consented' : 'CONSENTED');
        const defaultVote = director?.vote ?? (templateVersion === 1 ? 'For' : 'FOR');

        return (
          <div key={`${slot.roleKey}-${slot.roleIndex}`} className="grid gap-3 rounded-md border p-4 md:grid-cols-4">
            <Field
            ...
            <SelectField
              ...
              options={includeCurrentOption(templateVersion === 1 ? V1_VOTE_OPTIONS : V2_VOTE_OPTIONS, defaultVote)}
            />
          </div>
        );
      })}
