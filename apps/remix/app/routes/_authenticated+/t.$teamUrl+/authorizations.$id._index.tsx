...
import { canExecuteTeamAction, formatAuthorizationsPath, formatDocumentsPath } from '@documenso/lib/utils/teams';
...
import { formatAuthorizationDate, formatAuthorizationDateTime } from '~/utils/authorization-date-format';
...
export default function AuthorizationDetailPage({ loaderData }: Route.ComponentProps) {
  ...
  const canGenerate =
    canManage && authorization.status === ExecutiveAuthorizationStatus.DRAFT && !authorization.envelope;
  const canSend =
    canManage && authorization.status === ExecutiveAuthorizationStatus.READY && Boolean(authorization.envelope);
        ...
        <div className="space-y-6">
          <Card className="p-6">
            ...
            <dl className="mt-4 space-y-3 text-sm">
              ...
              {authorization.sentAtDisplay && <Detail label="Sent" value={authorization.sentAtDisplay} />}
            ...
            <div className="mt-4 space-y-3">
              {signers.map((signer, index) => (
                <div key={index} className="rounded-md border p-3 text-sm">
                  ...
                  {signer.signingUrl && (
                    <a
                      className="mt-2 inline-flex items-center gap-1 text-documenso-700 text-xs hover:opacity-80"
            ...
            <Card className="p-6">
              ...
              <div className="mt-4 space-y-2">
                {authorization.artifactLinks.map((artifact) => (
                  <Button key={artifact.key} asChild variant="outline" className="w-full justify-start">
