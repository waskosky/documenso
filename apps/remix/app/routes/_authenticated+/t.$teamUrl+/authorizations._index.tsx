...
export function meta() {
  return appMetaTags('Authorizations');
}
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            <Trans>
              Prepare board approvals, permission records, and executive decisions for signature
              while keeping the source details editable and easy to audit.
            </Trans>
          </p>
        </div>
      ...
      <div className="mt-8">
        {authorizations.length === 0 ? (
          <Card className="p-8">
            ...
            <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
              <Trans>
                Create the first authorization record to generate a board certificate, track
                signers, and preserve the decision context for future agents.
              </Trans>
            </p>
          </Card>
        ) : (
                ...
                {authorizations.map((authorization) => (
                  <tr key={authorization.id} className="border-t">
                    <td className="px-4 py-3">
                      ...
                      <div className="mt-1 text-xs text-muted-foreground">{authorization.type}</div>
                    </td>
