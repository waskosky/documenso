...
export async function action({ params, request }: Route.ActionArgs) {
  ...
  try {
    const authorization = await createExecutiveAuthorization({
      ...buildBoardAuthorizationInputFromFormData(formData, template.signing.signerRoles, template.version),
      teamId: team.id,
      userId: user.id,
    });
    ...
    <div className="mx-auto w-full max-w-screen-lg px-4 md:px-8">
      ...
      {loaderData.profileNeedsUpgrade && (
        <Alert className="mb-6">
          ...
          <AlertDescription>
            The saved defaults use an earlier certificate version. Review and save the current fields in{' '}
