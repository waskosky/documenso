...
const buildBoardDirectorsFromFormData = (
  ...
  buildAuthorizationSignerSlots(signerRoles)
    ...
    .map((slot) => ({
      ...
      vote:
        getString(formData, getAuthorizationSignerFieldName(slot, 'vote')) || (templateVersion === 1 ? 'For' : 'FOR'),
    }))
...
export const buildBoardAuthorizationProfileInputFromFormData = (
  ...
  return {
    actionMethod: getString(formData, 'actionMethod') as BoardResolutionCertificatePayload['actionMethod'],
