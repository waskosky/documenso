...
```bash
openssl pkcs12 -export -legacy -out certificate.p12 -inkey private.key -in certificate.crt
```

<Callout type="warn">
  You must set a password when prompted. Certificates without passwords, or modern OpenSSL 3
  PKCS#12 exports that the signer library cannot parse, cause signing failures with the error
