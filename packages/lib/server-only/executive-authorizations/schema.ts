...
export const ZBoardDirectorVoteSchema = z.enum([
  'ABSTAIN',
  'AGAINST',
  'FOR',
  'NOT_VOTING',
...
export const ZBoardResolutionCertificatePayloadV2BaseSchema = z
  .object({
    ...
    directors: z
      .array(ZBoardDirectorV2Schema)
      .length(3, 'Profile requires exactly 3 Director signers.'),
    entityType: z.string().trim().min(1).default('corporation'),
...
export const validateBoardExecutionRoleAssignments = (
  ...
  if (
    secretaryDirector &&
    normalizeName(secretaryDirector.name) !== normalizeName(payload.secretaryName)
  ) {
    context.addIssue({
...
export const validateBoardDecisionConsistency = (
  ...
  const participatingDirectors = payload.directors.filter(
    (director) => director.presence === expectedPresence,
  );
  ...
  const allParticipatingDirectorsVotedFor =
    participatingDirectors.length === payload.directors.length &&
    approvalCount === payload.directors.length;
  ...
  if (
    payload.actionMethod === 'MEETING' &&
    participatingDirectors.length < payload.quorumRequiredCount
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
  ...
  if (
    payload.actionMethod === 'MEETING' &&
    payload.directors.some((director) => director.presence === 'CONSENTED')
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
  ...
  if (
    payload.actionMethod !== 'MEETING' &&
    payload.directors.some((director) => director.presence === 'PRESENT')
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
  ...
  if (
    payload.directors.some(
      (director) => director.presence === 'ABSENT' && director.vote !== 'NOT_VOTING',
    )
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
  ...
  if (
    payload.resolutionDisposition === 'APPROVED_UNANIMOUSLY' &&
    !allParticipatingDirectorsVotedFor
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      message:
        'An approved-unanimously disposition requires every director to participate and vote FOR.',
      path: ['resolutionDisposition'],
    });
  }

  if (
    payload.resolutionDisposition === 'APPROVED_REQUIRED_VOTE' &&
    approvalCount < payload.approvalRequiredCount
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
...
export const ZBoardResolutionCertificatePayloadSchema =
  ZBoardResolutionCertificatePayloadV2BaseSchema.superRefine((payload, context) => {
...
export const parseAuthorizationTemplatePayload = ({
  ...
  if (!schema) {
    throw new Error(
      `Authorization payload schema version ${templateVersion} is not registered for "${templateKey}".`,
    );
  }
