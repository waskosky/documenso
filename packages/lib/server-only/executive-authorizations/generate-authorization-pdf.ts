...
export const generateAuthorizationPdf = async ({
  ...
  if (templateVersion >= 2) {
    ...
    const authorizedOfficer = signers.find((signer) =>
      signer.executionRoles?.includes('Authorized Officer'),
    );

    if (!secretary || !authorizedOfficer) {
      throw new Error(
        'Template version 2 requires Secretary and Authorized Officer director assignments.',
      );
    }
