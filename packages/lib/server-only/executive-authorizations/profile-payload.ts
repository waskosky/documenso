...
const validateProfileDirectors = (
  ...
  const duplicateEmail = findDuplicateAuthorizationSignerEmail(directors.map((director) => director.email ?? ''));
...
const ZBoardResolutionCertificateProfilePayloadV1Schema = ZBoardResolutionCertificatePayloadV1Schema.pick({
...
const ZBoardResolutionCertificateProfilePayloadSchema = ZBoardResolutionCertificatePayloadV2BaseSchema.pick({
  actionMethod: true,
...
export function parseAuthorizationTemplateProfilePayload<TTemplateKey extends AuthorizationTemplateKey>(arguments_: {
  payload: unknown;
  templateKey: TTemplateKey;
  templateVersion: number;
}): AuthorizationTemplateProfilePayloadMap[TTemplateKey];
export function parseAuthorizationTemplateProfilePayload<TTemplateKey extends AuthorizationTemplateKey>({
  payload,
  templateKey,
  templateVersion,
}: {
  ...
  if (!schema) {
    throw new Error(`Authorization profile schema version ${templateVersion} is not registered for "${templateKey}".`);
  }
